                      ...
                      <table id="myTable" class="w-full table-auto border border-gray-300 rounded-lg">
                        ...
                        <tbody>
                            ...
                            <tr class="border-b border-gray-200">
                                ...
                                <td class="p-3 border-r border-gray-300">{{ \Carbon\Carbon::parse($data->user->tanggal_lahir)->age }} Tahun</td>
                                ...
                                <td class="p-3 text-center flex justify-center items-center">
                                    ...
                                    <!-- Tombol Pemeriksaan -->
                                    <button onclick="openModal('{{ $data->id }}', '{{ $data->user->tekanan_darah }}', '{{ $data->user->frekuensi_nadi }}', '{{ $data->user->frekuensi_napas }}', '{{ $data->user->suhu_tubuh }}', '{{ $data->user->berat_badan }}', '{{ $data->user->tinggi_badan }}')">
                                        <img src="{{ asset('assets/icon/konfir.svg') }}" class="w-6 h-6" alt="Pemeriksaan">
                                    </button>
                                </td>
                            </tr>
                            @endforeach
...
<div id="editPendaftarModal" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 hidden z-50">
    <div class="bg-white p-6 rounded-lg shadow-lg w-1/3 max-w-lg relative">
        ...
        <form id="editPendaftarForm" method="POST" action="">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="tekanan_darah" name="tekanan_darah" class="w-full p-2 border rounded" placeholder="Contoh: 120/80 mmHg">
            </div>

            <div class="mb-3">
                <label for="frekuensi_nadi" class="block text-sm font-medium">Frekuensi Nadi (HR)</label>
                <input type="text" id="frekuensi_nadi" name="frekuensi_nadi" class="w-full p-2 border rounded" placeholder="Contoh: 75 bpm">
            </div>

            <div class="mb-3">
                <label for="frekuensi_napas" class="block text-sm font-medium">Frekuensi Napas (RR)</label>
                <input type="text" id="frekuensi_napas" name="frekuensi_napas" class="w-full p-2 border rounded" placeholder="Contoh: 18 rpm">
            </div>

            <div class="mb-3">
                <label for="suhu_tubuh" class="block text-sm font-medium">Suhu Tubuh</label>
                <input type="text" id="suhu_tubuh" name="suhu_tubuh" class="w-full p-2 border rounded" placeholder="Contoh: 36.5°C">
            </div>

            <div class="mb-3">
                <label for="berat_badan" class="block text-sm font-medium">Berat Badan (BB)</label>
                <input type="text" id="berat_badan" name="berat_badan" class="w-full p-2 border rounded" placeholder="Contoh: 65 Kg">
            </div>

            <div class="mb-3">
                <label for="tinggi_badan" class="block text-sm font-medium">Tinggi Badan (TB)</label>
                <input type="text" id="tinggi_badan" name="tinggi_badan" class="w-full p-2 border rounded" placeholder="Contoh: 170 cm">
            </div>
            ...
            <div class="flex justify-end mt-4">
                <button type="button" onclick="closeModal()" class="px-4 py-2 bg-gray-500 text-white rounded mr-2" style="background-color: #ea3370;">Batal</button>
