...
<div class="w-full max-w-2xl mx-auto">
    <div class="relative flex flex-wrap items-center bg-white shadow-lg rounded-2xl p-6">
        {{-- <img class="w-16 h-16 rounded-full" src="https://as2.ftcdn.net/v2/jpg/03/09/68/41/1000_F_309684117_g9WkhNf5TnsNNi3jtGSBQH8xbSwV8rof.jpg" alt="Profile Picture"> --}}
        <div class="ml-4">
    ...
    <div class="bg-white shadow-lg rounded-2xl p-6 mt-6">
        ...
        <form action="{{ route('profil.update') }}" method="POST">
            ...
            <input type="text" name="tempat_lahir" value="{{ old('tempat_lahir', $user->tempat_lahir) }}" class="w-full p-2 border rounded">

            <label class="block mt-4 text-md font-bold">Tanggal Lahir</label>
            <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $user->tanggal_lahir) }}" class="w-full p-2 border rounded">

            <label class="block mt-4 text-md font-bold">Nomor Telepon</label>
            <input type="text" name="nomor_telepon" value="{{ old('nomor_telepon', $user->nomor_telepon) }}" class="w-full p-2 border rounded">

            <label class="block mt-4 text-md font-bold">Alamat</label>
            <input type="text" name="alamat" value="{{ old('alamat', $user->alamat) }}" class="w-full p-2 border rounded">
            ...
            <button type="submit" class="mt-4 px-4 py-2 text-white rounded-lg " style="background-color: #709972;" >Update Profil</button>
        </form>
    </div>
</div>
@endsection
